...
async function createSupabaseLoginLink(
  ...
  const email = await createSyntheticKakaoEmail(subject, readRequiredEnv('KAKAO_IDENTITY_SECRET'))
  await admin.auth.admin.createUser({
    email,
    email_confirm: true,
    user_metadata: { provider: 'kakao', display_name: displayName },
  })
...
export async function handleKakaoOauthCallback(request: Request): Promise<Response> {
  ...
  try {
    isAllowed = await consumeRequestRateLimit(
      ...
      { bucket: 'kakao-oauth-callback', limit: 30, windowSeconds: 600 },
    )
  } catch {
  ...
  try {
    const profile = await fetchKakaoProfile(code, {
      clientId: readRequiredEnv('KAKAO_REST_API_KEY'),
      clientSecret: readRequiredEnv('KAKAO_CLIENT_SECRET'),
      redirectUri,
    })
    const actionLink = await createSupabaseLoginLink(profile.subject, profile.displayName, statePayload.returnTo)
    logOperationalEvent('info', 'kakao_oauth_succeeded', { requestId, status: 'redirected' })
    return new Response(null, {
      status: 302,
...
if (import.meta.main) Deno.serve(handleKakaoOauthCallback)
