alter type public.member_role add value if not exists 'manager' after 'owner';
...
grant execute on function private.can_manage_room_content(uuid) to authenticated;
...
create or replace function private.enforce_book_chat_manager_role()
returns trigger
language plpgsql
security definer
set search_path = pg_catalog, public, private
as $$
begin
  if not private.can_manage_room_content(new.room_id) then
    raise exception using errcode = 'P0001', message = 'ROOM_FORBIDDEN';
  end if;
  return new;
end;
...
before insert or update of status on public.book_chats
for each row execute function private.enforce_book_chat_manager_role();

revoke all on function private.enforce_book_chat_manager_role() from public, anon, authenticated;
comment on function private.enforce_book_chat_manager_role() is '책 대화 생성과 상태 변경을 방장 또는 운영자로 제한한다.';
