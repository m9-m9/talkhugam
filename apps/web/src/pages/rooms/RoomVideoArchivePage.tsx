  ...
  return (
    <main className="app-page bg-surface px-4 pb-8">
      ...
      <header className="mt-8">
        ...
        <p className="text-ink-subtle mt-2 text-sm">읽었던 모든 책의 영상을 한곳에서 볼 수 있어요.</p>
      </header>
      {videosQuery.isPending ? (
        <div className="mt-12"><BookLoadingIndicator label="영상을 불러오고 있어요." /></div>
      ) : videosQuery.isError ? (
        <div className="mt-8"><RetryState message="영상 기록을 불러오지 못했어요." onRetry={() => void videosQuery.refetch()} /></div>
          ...
          {videosQuery.data.map((video) => (
            <li key={video.id}>
              <ActionButton
                ...
                onClick={() => void navigate(`/rooms/${roomId}/books/${video.bookChatId}/videos/${video.id}`)}
                size="large"
                type="button"
                variant="neutralWeak"
              >
                <span className="min-w-0">
                  ...
                  <span className="text-ink mt-1 block text-sm font-semibold">{video.authorName}님의 영상</span>
                  <span className="text-ink-subtle mt-1 block text-xs">{new Date(video.createdAt).toLocaleDateString('ko-KR')}</span>
                </span>
              </ActionButton>
            </li>
          ))}
        ...
        <div className="talkhugam-information-surface border-ink/10 mt-8 rounded-lg border p-6 text-center">
          ...
          <p className="text-ink-subtle mt-2 text-sm">책 대화의 영상 기록에서 첫 순간을 남겨 보세요.</p>
        </div>
      )}
    </main>
  )
