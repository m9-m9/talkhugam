...
방장은 입력창 옆 `+` 메뉴의 `책방 초대하기`에서 초대 코드와 링크를 만들고 카카오톡·문자·인스타그램·페이스북 또는 복사로 공유할 수 있다. 같은 공유 시트는 `/rooms/:roomId/manage`의 초대 코드 만들기 뒤에도 바로 열린다. 인스타그램은 웹에서 메시지 자동 입력을 지원하지 않아 먼저 문구를 복사한 뒤 앱에서 붙여 넣도록 안내한다.
