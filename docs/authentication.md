...
| --- | --- | --- | --- |
| Kakao | Talk후감 Edge Function bridge | `kakao-oauth-callback` | 카카오 subject의 HMAC 결과를 내부 전용 이메일로 사용 |
...
1. 브라우저가 `{provider}-oauth-start?return_to=...`로 이동한다.
2. 서버가 `return_to`를 정확 일치 allowlist로 검사한다.
3. 서버가 10분짜리 state를 만들고 HMAC 서명한 HttpOnly, SameSite=Lax cookie를 설정한다.
4. Kakao 또는 Naver 동의 화면에서 callback으로 돌아오면 state, 서명, 만료를 모두 검사한다.
5. token 요청의 client secret은 URL이 아닌 POST form 본문으로 전송한다.
6. 공급자 subject는 HMAC 처리해 `@oauth.talkhugam.invalid` 내부 이메일로 변환한다.
7. Supabase Admin API가 사용자를 준비하고 일회성 magic link를 생성한다.
8. 브라우저는 Supabase action link를 거쳐 허용된 `return_to`로 돌아간다.

카카오 이메일, 네이버 이메일·전화번호·생일·성별은 Phase 1 로그인에 필요하지 않으므로 요청·저장하지 않는다.
Kakao bridge는 `profile_nickname`, `profile_image`만 요청한다. 그래서 카카오계정 이메일에 권한이 없거나
비즈앱이 아니어도 로그인할 수 있다.
...
| key | 보관 위치 | 회전 시 영향 |
| --- | --- | --- |
| `KAKAO_REST_API_KEY` | Edge Function secret | Kakao 앱 설정과 함께 변경 |
| `KAKAO_CLIENT_SECRET` | Edge Function secret | token 교환이 중단되므로 동시 교체 필요 |
| `KAKAO_REDIRECT_URI` | Edge Function secret/config | Kakao 개발자 콘솔 callback과 정확히 일치해야 함 |
| `KAKAO_STATE_SECRET` | Edge Function secret | 진행 중인 10분 이내 로그인만 무효화 |
| `KAKAO_IDENTITY_SECRET` | 장기 보관 secret | 변경하면 기존 Kakao 사용자를 찾지 못하므로 임의 회전 금지 |
...
`KAKAO_IDENTITY_SECRET`, `NAVER_IDENTITY_SECRET`은 암호 유출 대응이나 명시적 migration 없이 바꾸지 않는다. 회전이 필요하면
이전 key로 만든 내부 identity를 새 key로 이전하는 계획을 먼저 작성한다.

## 실제 연결 체크리스트

- [ ] Supabase 프로젝트의 Google provider client ID/secret 등록
- [ ] Kakao 개발자 콘솔 callback URL과 `KAKAO_REDIRECT_URI` 일치
- [ ] Kakao 제공 정보는 닉네임과 프로필 이미지에 필요한 최소 항목만 동의 요청
